...
//        $options['src'] = Url::to($url);
        $options['src'] = $url;
        if ($condition = ArrayHelper::remove($options, 'condition')) {
     ...
     * @see In::enableCsrfValidation
     */
    public static function csrfMetaTags()
    {
        $in = App::$in;
        if ($in instanceof In && $in->enableCsrfValidation) {
            return static::tag('meta', '', ['name' => 'csrf-param', 'content' => $in->csrfParam]) . "\n    "
                . static::tag('meta', '', ['name' => 'csrf-token', 'content' => $in->getCsrfToken()]) . "\n";
        } else {
            return '';
        }
    }
...
//                $hiddenInputs[] = static::hiddenInput($in->methodParam, $method);
//                $method = 'post';
//            }
            if ($in->enableCsrfValidation && !strcasecmp($method, 'post')) {
                $hiddenInputs[] = static::hiddenInput($in->csrfParam, $in->getCsrfToken());
            }
        }
        ...
        if (isset($options['autocomplete']) && $options['autocomplete'] === 'off') {
            $hiddenInputs[] = '<input style="display:none"><input type="password" style="display:none">';
        }
    ...
    {
        if ($url !== null) {
            $options['href'] = App::createUrl($url);
        }
...
//        $options['src'] = Url::to($src);
    ...
    public static function radio($name, $checked = false, $options = [])
    {
        ...
        $labelCheckClass = ArrayHelper::remove($options, 'labelCheckClass', '');
        if (isset($options['label'])) {
            $label = $options['label'];
            $labelOptions = isset($options['labelOptions']) ? $options['labelOptions'] : [];
            unset($options['label'], $options['labelOptions']);
            if ($labelCheckClass && $checked) {
                $labelOptions['class'] = (isset($labelOptions['class']) ? $labelOptions['class'].' ' : '').$labelCheckClass;
            }
    ...
    {
        $header = isset($options['header']) ? $options['header'] : '<p>' . App::t('mvc', 'Please fix the following errors:') . '</p>';
