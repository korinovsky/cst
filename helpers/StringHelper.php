...
class StringHelper
{
    ...
    {
//        if ($asHtml) {
//            return static::truncateHtml($string, $length, $suffix, $encoding ?: 'UTF-8');
//        }
...
//    }
    ...
    public static function endsWith($string, $with, $caseSensitive = true)
    {
        ...
        if ($caseSensitive) {
            ...
            return mb_strtolower(mb_substr($string, -$bytes, null, '8bit'), 'UTF-8') === mb_strtolower($with, 'UTF-8');
        }
    }
     ...
     * The string generated matches [A-Z0-9]+ and is transparent to URL-encoding.
     *
     * @param integer $length the length of the key in characters
     * @param integer $chars characters
     * @return string the generated random key
     * @throws InvalidConfigException if OpenSSL extension is needed but not installed.
     * @throws Exception on failure.
     */
    public static function generateRandomString($length = 32, $chars = 'ABCDEFGHIJKLMOPQRSTUVXWYZ0123456789')
    {
        $clenght = strlen($chars);
        $clenght--;
        $result = null;
        for($x = 1; $x <= $length; $x++){
            $pos = rand(0, $clenght);
     ...
     * @throws Exception on failure.
     */
    public static function generateRandomNumber($length = 32)
    {
        return static::generateRandomString($length, '0123456789');
    }

    /**
     * Экранирует необходимые символы для запроса LIKE в бд
     * @param string $value Строка
     * @return string Возвращает обработанную строку
     */
    public static function dbLikeEscape($value) {
    ...
    public static function minutesPer($i) {
        $d = $h = 0;
        if ($i > 59) {
            $h = intval($i / 60);
            $i = $i % 60;
        ...
        $result = [];
        if ($d >= 1) {
            $result[] = App::t('mvc', '{delta, plural, =1{a day} other{# days}}', ['delta' => $d]);
        }
        if ($h >= 1) {
            $result[] = App::t('mvc', '{delta, plural, =1{an hour} other{# hours}}', ['delta' => $h]);
        }
        if ($i >= 1) {
            $result[] = App::t('mvc', '{delta, plural, =1{a minute} other{# minutes}}', ['delta' => $i]);
        }
        return $result ? App::t('mvc', 'per {0}', [implode(' ', $result)]) : '';
    }}
