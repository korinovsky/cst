...
use cst\base\ViewContextInterface;
use cst\web\View;
...
abstract class BaseMailer extends Object implements MailerInterface, ViewContextInterface
{
     ...
     */
    public $messageClass = 'cst\mail\BaseMessage';
    /**
     ...
     */
    public $fileTransportPath = RUNTIME_PATH.'mail';
    /**
    ...
    /**
     * @var \cst\base\View|array view instance or its array configuration.
     */
    private $_view = [];
    /**
    ...
    protected function createView(array $config)
    {
        ...
        return App::createObject($config);
    }
    ...
    public function send($message)
    {
        ...
        App::info('Sending email "' . $message->getSubject() . '" to "' . $address . '"', __METHOD__);
     ...
     * @param MessageInterface|Message $message the message to be sent
    ...
    {
        if (!is_dir($this->fileTransportPath)) {
            mkdir($this->fileTransportPath, 0777, true);
        }
        if ($this->fileTransportCallback !== null) {
            $file = $this->fileTransportPath . '/' . call_user_func($this->fileTransportCallback, $this, $message);
        } else {
            $file = $this->fileTransportPath . '/' . $this->generateMessageFileName();
        }
