...
class I18N extends Object
{
    ...
    public function format($message, $params, $language)
    {
        ...
        if (preg_match('~{\s*[\d\w]+\s*,~u', $message)) {
            ...
            if ($result === false) {
                ...
                App::warning("Formatting message for language '$language' failed with error: $errorMessage. The message being formatted was: $message.", __METHOD__);
    ...
    public function getMessageFormatter()
    {
        if ($this->_messageFormatter === null) {
            ...
            $this->_messageFormatter = App::createObject($this->_messageFormatter);
        }
     ...
     * Can be given as array or string configuration that will be given to [[App::createObject]] to create an instance
    ...
    public function getMessageSource($category)
    {
        if (isset($this->translations[$category])) {
            ...
            if ($source instanceof MessageSource) {
                ...
                return $this->translations[$category] = App::createObject($source);
            }
        } else {
            ...
            foreach ($this->translations as $pattern => $source) {
                if (strpos($pattern, '*') > 0 && strpos($category, rtrim($pattern, '*')) === 0) {
                    if ($source instanceof MessageSource) {
                        ...
                        return $this->translations[$category] = $this->translations[$pattern] = App::createObject($source);
                    }
                }
            }
            // match '*' in the last
            if (isset($this->translations['*'])) {
                ...
                if ($source instanceof MessageSource) {
                    ...
                    return $this->translations[$category] = $this->translations['*'] = App::createObject($source);
                }
            }
        }
