...
class Formatter extends Object
{
    ...
    /**
     * @var array Телефонные коды городов
     */
    private $phoneCodes3 = ['495', '499', '812'];
    ...
    public function init()
    {
        ...
        if ($this->booleanFormat === null) {
            $this->booleanFormat = [App::t('mvc', 'No', [], $this->locale), App::t('mvc', 'Yes', [], $this->locale)];
        }
        if ($this->nullDisplay === null) {
            $this->nullDisplay = '<span class="not-set">' . App::t('mvc', '(not set)', [], $this->locale) . '</span>';
        }
    ...
    private function formatDateTimeValue($value, $format, $type)
    {
        ...
        if ($this->_intlLoaded && !(PHP_INT_SIZE == 4 && ($year <= 1901 || $year >= 2038))) {
            ...
            return preg_match('/(^| )[M|L]{3}($| |,)/', $format) ? preg_replace('/(^| )([a-zа-я]{3})[a-zа-я\.]*/iu', '$1$2', $result) : $result;
        } else {
            if (strncmp($format, 'php:', 4) === 0) {
    ...
    public function asRelativeTime($value, $referenceTime = null)
    {
        ...
        if ($interval->invert) {
            if ($interval->y >= 1) {
                return App::t('mvc', 'in {delta, plural, =1{a year} other{# years}}', ['delta' => $interval->y], $this->locale);
            }
            if ($interval->m >= 1) {
                return App::t('mvc', 'in {delta, plural, =1{a month} other{# months}}', ['delta' => $interval->m], $this->locale);
            }
            if ($interval->d >= 1) {
                return App::t('mvc', 'in {delta, plural, =1{a day} other{# days}}', ['delta' => $interval->d], $this->locale);
            }
            if ($interval->h >= 1) {
                return App::t('mvc', 'in {delta, plural, =1{an hour} other{# hours}}', ['delta' => $interval->h], $this->locale);
            }
            if ($interval->i >= 1) {
                return App::t('mvc', 'in {delta, plural, =1{a minute} other{# minutes}}', ['delta' => $interval->i], $this->locale);
            }
            if ($interval->s == 0) {
                return App::t('mvc', 'just now', [], $this->locale);
            }
            return App::t('mvc', 'in {delta, plural, =1{a second} other{# seconds}}', ['delta' => $interval->s], $this->locale);
        } else {
            if ($interval->y >= 1) {
                return App::t('mvc', '{delta, plural, =1{a year} other{# years}} ago', ['delta' => $interval->y], $this->locale);
            }
            if ($interval->m >= 1) {
                return App::t('mvc', '{delta, plural, =1{a month} other{# months}} ago', ['delta' => $interval->m], $this->locale);
            }
            if ($interval->d >= 1) {
                return App::t('mvc', '{delta, plural, =1{a day} other{# days}} ago', ['delta' => $interval->d], $this->locale);
            }
            if ($interval->h >= 1) {
                return App::t('mvc', '{delta, plural, =1{an hour} other{# hours}} ago', ['delta' => $interval->h], $this->locale);
            }
            if ($interval->i >= 1) {
                return App::t('mvc', '{delta, plural, =1{a minute} other{# minutes}} ago', ['delta' => $interval->i], $this->locale);
            }
            if ($interval->s == 0) {
                return App::t('mvc', 'just now', [], $this->locale);
            }
            return App::t('mvc', '{delta, plural, =1{a second} other{# seconds}} ago', ['delta' => $interval->s], $this->locale);
        }
    }
    ...
    public function asCurrency($value, $currency = null, $options = [], $textOptions = [])
    {
        ...
        if ($this->_intlLoaded) {
            ...
            return preg_replace('/[.,]00(\D*)$/', '$1', $result);
        } else {
            if ($currency === null) {
                if ($this->currencyCode === null) {
    ...
    public function asShortSize($value, $decimals = null, $options = [], $textOptions = [])
    {
        ...
        if ($this->sizeFormatBase == 1024) {
            switch ($position) {
                case 0:  return App::t('mvc', '{nFormatted} B', $params, $this->locale);
                case 1:  return App::t('mvc', '{nFormatted} KiB', $params, $this->locale);
                case 2:  return App::t('mvc', '{nFormatted} MiB', $params, $this->locale);
                case 3:  return App::t('mvc', '{nFormatted} GiB', $params, $this->locale);
                case 4:  return App::t('mvc', '{nFormatted} TiB', $params, $this->locale);
                default: return App::t('mvc', '{nFormatted} PiB', $params, $this->locale);
            }
        } else {
            switch ($position) {
                case 0:  return App::t('mvc', '{nFormatted} B', $params, $this->locale);
                case 1:  return App::t('mvc', '{nFormatted} KB', $params, $this->locale);
                case 2:  return App::t('mvc', '{nFormatted} MB', $params, $this->locale);
                case 3:  return App::t('mvc', '{nFormatted} GB', $params, $this->locale);
                case 4:  return App::t('mvc', '{nFormatted} TB', $params, $this->locale);
                default: return App::t('mvc', '{nFormatted} PB', $params, $this->locale);
            }
        }
    }
    ...
    public function asSize($value, $decimals = null, $options = [], $textOptions = [])
    {
        ...
        if ($this->sizeFormatBase == 1024) {
            switch ($position) {
                case 0:  return App::t('mvc', '{nFormatted} {n, plural, =1{byte} other{bytes}}', $params, $this->locale);
                case 1:  return App::t('mvc', '{nFormatted} {n, plural, =1{kibibyte} other{kibibytes}}', $params, $this->locale);
                case 2:  return App::t('mvc', '{nFormatted} {n, plural, =1{mebibyte} other{mebibytes}}', $params, $this->locale);
                case 3:  return App::t('mvc', '{nFormatted} {n, plural, =1{gibibyte} other{gibibytes}}', $params, $this->locale);
                case 4:  return App::t('mvc', '{nFormatted} {n, plural, =1{tebibyte} other{tebibytes}}', $params, $this->locale);
                default: return App::t('mvc', '{nFormatted} {n, plural, =1{pebibyte} other{pebibytes}}', $params, $this->locale);
            }
        } else {
            switch ($position) {
                case 0:  return App::t('mvc', '{nFormatted} {n, plural, =1{byte} other{bytes}}', $params, $this->locale);
                case 1:  return App::t('mvc', '{nFormatted} {n, plural, =1{kilobyte} other{kilobytes}}', $params, $this->locale);
                case 2:  return App::t('mvc', '{nFormatted} {n, plural, =1{megabyte} other{megabytes}}', $params, $this->locale);
                case 3:  return App::t('mvc', '{nFormatted} {n, plural, =1{gigabyte} other{gigabytes}}', $params, $this->locale);
                case 4:  return App::t('mvc', '{nFormatted} {n, plural, =1{terabyte} other{terabytes}}', $params, $this->locale);
                default: return App::t('mvc', '{nFormatted} {n, plural, =1{petabyte} other{petabytes}}', $params, $this->locale);
            }
        }
    }
     ...
     * @return string Возвращает отформатированный номер телефона
     */
    public function asPhone($value) {
        if (isset($value) && $value = trim(preg_replace(['/\(|\)/', '/[–−—]/', '/^00/', '/^0/', '/[^0-9\s+-]/', '/\s{2,}/', '/-\s/', '/\s-/', '/-{2,}/'], [' ', '-', '+', '', '', ' ', '-', '-', '-'], $value))) {
            $numbers = preg_replace('/[^0-9]/', '', $value);
            // Российские сотовые
            if (preg_match('/^(?<code>[78]{0,1})(?<ph0>9[0-9]{2})(?<ph1>[0-9]{3})(?<ph2>[0-9]{2})(?<ph3>[0-9]{2})$/', $numbers, $matches)) {
                $value = '+7 '.$matches['ph0'].' '.$matches['ph1'].'-'.$matches['ph2'].'-'.$matches['ph3'];
            }
            // Казахские сотовые
            elseif (preg_match('/^7(?<ph0>7[04567][0-9])(?<ph1>[0-9]{3})(?<ph2>[0-9]{2})(?<ph3>[0-9]{2})$/', $numbers, $matches)) {
                $value = '+7 '.$matches['ph0'].' '.$matches['ph1'].'-'.$matches['ph2'].'-'.$matches['ph3'];
            }
            // Белорусские сотовые
            elseif (preg_match('/^375(?<ph0>25|29|33|44)(?<ph1>[0-9]{3})(?<ph2>[0-9]{2})(?<ph3>[0-9]{2})$/', $numbers, $matches)) {
                $value = '+375 '.$matches['ph0'].' '.$matches['ph1'].'-'.$matches['ph2'].'-'.$matches['ph3'];
            }
            // Украинские сотовые
            elseif (preg_match('/^380(?<ph0>39|50|63|66|67|68|91|92|93|94|95|96|97|98|99)(?<ph1>[0-9]{3})(?<ph2>[0-9]{2})(?<ph3>[0-9]{2})$/', $numbers, $matches)) {
                $value = '+380 '.$matches['ph0'].' '.$matches['ph1'].'-'.$matches['ph2'].'-'.$matches['ph3'];
            }
            // Белорусские и украинские городские
            elseif (preg_match('/^(?<code>375|380)(?<ph0>[0-9]{9})$/', $numbers, $matches)) {
                $value = '+'.$matches['code'].' '.$matches['ph0'];
            }
            // Российские городские с кодом
            elseif (preg_match('/^(?<code>[78]{0,1})(?<ph0>[1-9][0-9]{2})(?<ph1>[0-9]{3})(?<ph2>[0-9]{2})(?<ph3>[0-9]{2})$/', $numbers, $matches)) {
                if (in_array($matches['ph0'], $this->phoneCodes3)) {
                    $value = '+7 '.$matches['ph0'].' '.$matches['ph1'].'-'.$matches['ph2'].'-'.$matches['ph3'];
                }
                else {
                    $value = '+7 '.$matches['ph0'].$matches['ph1'].$matches['ph2'].$matches['ph3'];
            ...
            // Российские городские без кода
            elseif (preg_match('/^(?<ph0>[0-9]{1,3})(?<ph1>[0-9]{2})(?<ph2>[0-9]{2})$/', $numbers, $matches)) {
                $value = $matches['ph0'].'-'.$matches['ph1'].'-'.$matches['ph2'];
            }
        }
        return $value;
    }
