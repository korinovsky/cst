...
class Command extends Object
{
    ...
    /**
     * @var \cst\caching\Dependency the dependency to be associated with the cached query result for this command
     * @see cache()
     */
     ...
     * @param \cst\caching\Dependency $dependency the cache dependency associated with the cached query result.
     ...
     * @param array|\cst\db\Query $columns the column data (name => value) to be inserted into the table or instance
     * of [[cst\db\Query|Query]] to perform INSERT INTO ... SELECT SQL statement.
     * Passing of [[cst\db\Query|Query]] is available since version 2.0.11.
     * @return $this the command object itself
     */
     ...
     * @param string $type the column type. [[\cst\db\QueryBuilder::getColumnType()]] will be called
    ...
    public function execute()
    {
        ...
        try {
            $profile and App::beginProfile($rawSql, __METHOD__);

            $this->pdoStatement->execute();
            $n = $this->pdoStatement->rowCount();

            $profile and App::endProfile($rawSql, __METHOD__);
    ...
    {
        list($profile, $rawSql) = $this->logQuery('cst\db\Command::query');

        if ($method !== '') {
            $info = $this->db->getQueryCacheInfo($this->queryCacheDuration, $this->queryCacheDependency);
            if (is_array($info)) {
                /* @var $cache \cst\caching\Cache */
                $cache = $info[0];
                $cacheKey = [
                    __CLASS__,
                    $method,
                ...
                if (is_array($result) && isset($result[0])) {
                    App::trace('Query result served from cache', 'cst\db\Command::query');
                    return $result[0];
                }
            }
        }
        ...
        try {
            $profile and App::beginProfile($rawSql, 'cst\db\Command::query');
            ...
            $profile and App::endProfile($rawSql, 'cst\db\Command::query');
        } catch (\Exception $e) {
            $profile and App::endProfile($rawSql, 'cst\db\Command::query');
            throw $this->db->getSchema()->convertException($e, $rawSql ?: $this->getRawSql());
        }
        ...
        if (isset($cache, $cacheKey, $info)) {
            ...
            App::trace('Saved query result in cache', 'cst\db\Command::query');
        }
