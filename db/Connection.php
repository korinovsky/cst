...
use cst\base\NotSupportedException;
use cst\caching\Cache;
 ...
 *
 * ```php
 * $connection = new \cst\db\Connection([
 *     'dsn' => $dsn,
 *     'username' => $username,
 *     'password' => $password,
 * ]);
...
class Connection extends Object
{
     ...
     * schema class name or configuration. Please refer to [[App::createObject()]] for
     * details on how to specify a configuration.
     *
     * This property is mainly used by [[getSchema()]] when fetching the database schema information.
     * You normally do not need to set this property unless you want to use your own
     * [[Schema]] class to support DBMS that is not supported by App.
     */
    public $schemaMap = [
        'pgsql' => 'cst\db\pgsql\Schema', // PostgreSQL
        'mysqli' => 'cst\db\mysql\Schema', // MySQL
        'mysql' => 'cst\db\mysql\Schema', // MySQL
        'sqlite' => 'cst\db\sqlite\Schema', // sqlite 3
        'sqlite2' => 'cst\db\sqlite\Schema', // sqlite 2
        'sqlsrv' => 'cst\db\mssql\Schema', // newer MSSQL driver on MS Windows hosts
        'oci' => 'cst\db\oci\Schema', // Oracle driver
        'mssql' => 'cst\db\mssql\Schema', // older MSSQL driver on MS Windows hosts
        'dblib' => 'cst\db\mssql\Schema', // dblib drivers on GNU/Linux (and maybe other OSes) hosts
        'cubrid' => 'cst\db\cubrid\Schema', // CUBRID
    ];
    /**
     * @var string Custom PDO wrapper class. If not set, it will use [[PDO]] or [[\cst\db\mssql\PDO]] when MSSQL is used.
     * @see pdo
     */
    public $pdoClass;
    /**
     ...
     * @since 2.0.7
     */
    public $commandClass = 'cst\db\Command';
    /**
    ...
    /**
     * @var Schema|Object the database schema
     */
    private $_schema;
    /**
     ...
     * Use 0 to indicate that the cached data will never expire.
     * @param \cst\caching\Dependency $dependency the cache dependency associated with the cached query results.
     ...
     * @param \cst\caching\Dependency $dependency the preferred caching dependency. If null, it will be ignored.
    ...
    public function getQueryCacheInfo($duration, $dependency)
    {
        ...
        if ($duration === 0 || $duration > 0) {
            if (is_string($this->queryCache)) {
                $cache = App::getComponent($this->queryCache, false);
            } else {
    ...
    public function open()
    {
        ...
        try {
            App::info($token, __METHOD__);
            App::beginProfile($token, __METHOD__);
            $this->pdo = $this->createPdoInstance();
            $this->initConnection();
            App::endProfile($token, __METHOD__);
    ...
    public function close()
    {
        ...
        if ($this->pdo !== null) {
            App::trace('Closing DB connection: ' . $this->dsn, __METHOD__);
            $this->pdo = null;
            $this->_schema = null;
        ...
        if ($pdoClass === null) {
            ...
            if (isset($driver)) {
                if ($driver === 'mssql' || $driver === 'dblib') {
                    $pdoClass = 'cst\db\mssql\PDO';
                } elseif ($driver === 'sqlsrv') {
                    $pdoClass = 'cst\db\mssql\SqlsrvPDO';
                }
            }
        }
     ...
     * from rollback will be caught and just logged with [[\App::error()]].
    ...
    public function getSchema()
    {
        if ($this->_schema !== null) {
            ...
            if (isset($this->schemaMap[$driver])) {
                ...
                return $this->_schema = App::createObject($config);
            } else {
    ...
    protected function openFromPoolSequentially(array $pool, array $sharedConfig)
    {
        ...
        $cache = is_string($this->serverStatusCache) ? App::getComponent($this->serverStatusCache, false) : $this->serverStatusCache;
