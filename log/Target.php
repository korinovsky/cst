...
abstract class Target extends Object
{
     ...
     * match those categories sharing the same common prefix. For example, 'cst\db\*' will match
     * categories starting with 'cst\db\', such as 'cst\db\Connection'.
     */
    public $categories = [];
    /**
    ...
    public function collect($messages, $final)
    {
        ...
        if ($count > 0 && ($final || $this->exportInterval > 0 && $count >= $this->exportInterval)) {
            if (($context = $this->getContextMessage()) !== '') {
                $this->messages[] = [$context, Logger::LEVEL_INFO, 'app', BEGIN_TIME];
            }
    ...
    public function getMessagePrefix($message)
    {
        ...
        $ip = $userID = $sessionID = '-';
        if (App::$controller instanceof \cst\web\Controller) {
            $ip = App::$in->getUserIP();
            if (!App::$controller->user->isGuest) {
                $userID = App::$controller->user->id;
            }
            if (App::$controller->session->isActive) {
                $sessionID = App::$controller->session->getId();
            }
        }
        return "[$ip][$sessionID][$userID]";
    }
}
