 ...
 *
 * A Logger instance can be accessed via `App::getLogger()`. You can call the method [[log()]] to record a single log message.
...
class Logger extends Object
{
    ...
    public function log($message, $level, $category = 'application')
    {
        ...
        if ($this->traceLevel > 0) {
            ...
            foreach ($ts as $trace) {
                if (isset($trace['file'], $trace['line']) && strpos($trace['file'], CST_PATH) !== 0) {
    ...
    public function flush($final = false)
    {
        ...
        if (is_array($this->dispatcher)) {
            $this->dispatcher['class'] = 'cst\log\Dispatcher';
            $this->dispatcher = App::createObject($this->dispatcher);
        }
     ...
     * defined by constant `BEGIN_TIME` which is evaluated at the beginning
     * of [[App]] class file.
     ...
     * For example, 'cst\db\*' will match categories starting with 'cst\db\',
     * such as 'cst\db\Connection'.
    ...
    {
        $timings = $this->getProfiling(['cst\db\Command::query', 'cst\db\Command::execute']);
        $count = count($timings);
        $time = 0;
